...
How **attackmap** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `attackmap` lives in the **Threat intel / detections** cluster.
...
| from | relation | to |
|---|---|---|
| `attackmap` | JSON findings compose with | [`stixgen`](https://github.com/cognis-digital/stixgen), [`iocextract`](https://github.com/cognis-digital/iocextract), [`ttphunt`](https://github.com/cognis-digital/ttphunt) |
| `attackmap` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `attackmap` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `attackmap` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the threat intel / detections cluster.** `attackmap` output feeds the next tool:
```bash
attackmap ... --format json > out.json          # this tool's findings (see `attackmap --help`)
...
attackmap ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
attackmap ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
attackmap ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `attackmap`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `attackmap` slots
