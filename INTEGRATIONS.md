...
**attackmap** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `attackmap` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
attackmap ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
attackmap ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
attackmap ... --format json | cognis-connect emit --to sigma   # Sigma rules
attackmap ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
```
...
works whatever `attackmap` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="attackmap") for rec in records]   # records = your JSON output
